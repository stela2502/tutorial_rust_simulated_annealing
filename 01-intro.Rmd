...
The "i32" in Rusts variable definition tells the compiler that the variable x may only contain 32 bit unsigned iteger information - this is what **strong type savety** does mean in reality. If you ommit this the compiler will choose a default variable thype - here "usize" - a computer architecture specific unsigned interger - on 64bit processors a u64. Meaning this variable would use double the memory from the u32.
...
We will create a class Cluster that contains a centroid (two float values) and a members vector, along with a single function that prints the centroid.
In R, this will be implemented as an S3 class - the most simple 'class' definition in R.
In Rust, this will be implemented as a Rust struct.
...
```rust
# Define the Rust struct
struct Cluster {
    centroid: (f64, f64), // a Tupel of 2 64bit wide 'float' variables
    members: Vec<u32>, // a vector of u32 variables - unsinged 32bit wide integers
}
