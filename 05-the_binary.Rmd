...
-- **shot** use the variable names first character for a short option like -f for file
...
Add this main function to that script and we are ready to go:
...
Add a store variable into the class and calculate the row to row distances after you have scaled the data.
Then modify the calc_ek() function to use the values from the store and not calculate the distances every single time.
This should give you an additional x5 speed improvement if you store it as a Vec<Vec<f32>> again. If you use a single vector and an index function you cold get an even better performance.
